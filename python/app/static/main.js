...
// for (const [key, value] of Object.entries(role_json))
...
function build_dataSet() {
    ...
    for (let role of role_json)
        role_map.set(role.name, role);    // i.e. role_map = new Map(role_json.map(key => [key.name, key]));
}
...
function load() {
    ...
    for (let role of role_map.values()) {
        ...
        document.getElementById(divId).innerHTML +=
            `&nbsp; <input type=checkbox id=fck_${role.name} onchange=checkboxChange_handler(event) >  ${role.name} <br>`;
...
function setDefaults() {
    ...
    for (item of document.querySelectorAll('[id^=fck_]'))
...
function show_hide_roles() {
    if (link1.innerText == 'More Roles') {
        link1.innerText = 'Less Roles';
        pCity2.style.setProperty('display', 'inline');
        pMafia2.style.setProperty('display', 'inline');
    }
    else {
        link1.innerText = 'More Roles';
